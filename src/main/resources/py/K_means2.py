...
class vis:
    def __init__(self,data,num_clustres):#传入数据date和簇的数量num_clustres
        self.data=data
        self.num_clustres=num_clustres
    def show(data,num_clustres,x_axis,y_axis):#传入数据date和簇的数量num_clustres,和用户指定的两个变量
        num_examples=data.shape[0]
        x_train=data[[x_axis,y_axis]].values.reshape(num_examples,2)#传入数据，两列
        ...
        plt.xlabel(x_axis)
        plt.ylabel(y_axis)
        plt.savefig("test.png")#保存图片，可以在这里设置存储路径
        #plt.show()
